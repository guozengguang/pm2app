...
var  Pay={
  ...
  },set_payorderotherinfo:function(req,res){
    var body=req.body;
  
    if(!body.openid){
      return response.ApiError(res,{message:"openid error"});
    }
    if(!body.tel){
      return response.ApiError(res,{message:"tel error"});
    }
   var content={};
        if (body.tel){
          content.po_userphone=body.tel;
        }
        if (body.name){
          content.po_username=body.name;
        }
        if (body.classname){
          content.po_userclassname=body.classname;
        }
       models.Payorder.update(content,{where:{po_openid:body.openid}}).then(function(){
          return response.onDataSuccess(res,{data:{}});
      }, function(err){
        return response.ApiError(res,{message:"update payorder error"});
      });  
  },get_payorderbyopenid:function(req,res){
    var body=req.query;
  
    if(!body.openid){
      return response.ApiError(res,{message:"openid error"});
    }
    models.Payorder.getpayorderbyopenid({
