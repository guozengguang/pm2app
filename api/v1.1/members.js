...
//models.Members.destroy({where:{m_type:0}})
var Members={
  app_auth:function(req,res){//用来获取token
    ...
    models.Members.findOne({
      where:{m_phone:m_mpno,m_status:1}
    }).then(function(item){
      if (item){
        // 产生验证码
    ...
    co(function* (){
      try{
        ...
        if(smscode){
          ...
          return response.ApiError(res,{message:"验证码错误，请重新输入验证码"});
        }
      }catch (err){
        console.log(err)
      }
    ...
    models.Members.getmemberinfo({
      where:where,
      attributes:['user_name','user_pics','user_position','user_phone','user_url','user_place','user_company','company_desc','user_email','goods_name','isover','ismyfriend','uc_calssroomname','user_sex']
    }).then(function(item){
      item.forEach( function(node, index) {
