...
var Question={
  list:function(req,res){
  ...
  },questionlist_outside:function(req,res){
    var body=req.query;
    var options=utils.get_page_options(req);
    var where={};
    if (!body.classid) {
    ...
    where.question_classid=body.classid;
    where.order=body.order;
    models.Question.getquestionsforoutside({
          where:where
     }).then(function(item){
      if (item) {
    ...
    try{
      co(function* (){
        ...
        var my_class=yield models.Class.findOne({
          where:{classid:body.classid},
          attributes:['class_qustart','class_start','class_quend','class_qustatus']
        });
        //
        if (my_class.dataValues.class_qustatus!=2) {//结束
    ...
    sql.AppendFormat("select question_classroomid from  gj_question where question_fromuser = {0} and question_classroomid>0",body.userid);
    var myquestions=yield models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT });
     if(myquestions.length>0){
        var classroomid=myquestions[0].question_classroomid;
        ...
        models.Question.create({
          ...
          question_classroomid:classroomid
        }).then(function(){
          return response.ApiSuccess(res,{})
        }, function(err){
          console.log(err)
      ...
      try{
         ...
         if(smscode){
            ...
            models.Question.create({
              ...
              question_classid:body.classid,
              question_content:body.content,
              question_classroomid:body.classroomid
            }).then(function(){
              return response.ApiSuccess(res,{})
            }, function(err){
              console.log(err)
