...
var Members={
  app_auth:function(req,res){//用来获取token
    ...
    var send_type=body.send_type?body.send_type:'common';
    models.Members.findOne({
      where:{m_phone:m_mpno}
    }).then(function(item){
      if (item || send_type=='apply'){//存在用户  或者登陆为报名登陆
        // 产生验证码
        ...
        sms.get_code({mpno:m_mpno,code:code},function(err,result){
          ...
          if (result.statusCode=="000000"){//发送成功
            ...
            return response.onDataSuccess(res,{data:'短信已发送'});
          }else {
            return response.ApiError(res,{message:"短信发送失败"});
          }
        });
      }else {
        return response.ApiError(res,{code:401,message:"非会员用户"});
      }
    }, function(err){
      console.log(err)
    ...
    co(function* (){
      var smscode=yield models.Smscode.findOne({
        where:{smscode:m_code,phoneno:m_mpno,createdAt:{'$gt': moment().add('-10','minute').format()}}
      })
      smscode=1
      if(smscode){
        models.Members.findOne({
        ...
        return response.ApiError(res,{message:"验证码无效"});
      }
    });
        ...
        if(member){
          ...
          member.dataValues.m_teacherrightimg=str.AbsolutePath(member.dataValues.m_teacherrightimg);
            ...
            if(userclass){
              ...
              var m_class=userclass.uc_areaname+goods.dataValues.goods_subtitle+userclass.uc_calssroomname;
    ...
    var body=req.body;
    if (!body.content || !body.fromuser) {
      return response.ApiError(res,{message:"goodsid or userid empty"});
    }
    models.Feedback.create({
      feedback_fromuser:body.fromuser,
      feedback_touser:body.touser,
    ...
    models.Members.getmemberinfo({
      where:where,
      attributes:['user_name','user_pics','user_position','user_phone','user_url','user_place','user_company','company_desc','user_email','goods_name','isover','ismyfriend']
    }).then(function(item){
      item.forEach( function(node, index) {
