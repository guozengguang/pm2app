...
var Class={
  list:function(req,res){
    ...
    co(function * (){
      try{
        ...
        var goods=yield models.Goods.findAndCountAll({
          where:where,
          order:[['createdAt']],
          attributes:['goodsid','goods_start','goods_end','goods_img','goods_name','goods_summary','goods_teacher'],
    ...
    co(function* () {
      try{
        var detail=yield models.Goods.findOne({
          ...
          order:[['goods_start']],
          attributes: ['goodsid', 'goods_name', 'goods_content','goods_img', 'goods_start','goods_teacher','goods_summary'],
        });
        if(body.userid){
          //存在用户id
        ...
        if (detail) {
          var Classes=yield models.Class.findAll({
            ...
            attributes: ['classid','class_goodsid', 'class_start', 'class_name', 'class_teacherid', 'class_qustart', 'class_asstart', 'class_end','class_img','class_content','class_summary'],
            order:[['class_start']]
          });
  ...
  },classitem_reward:function(req,res){
    var body=req.query;
    if(!body.classid){
  ...
  },classitem_detail:function(req,res){
    var body=req.query;
    if (!body.classid) {
      return response.ApiError(res,{message:"classid empty"});
    }
    co(function* (){
      try{
        var detail=yield models.Class.findOne({
          where:{classid:body.classid},
          attributes: ['classid','class_goodsid', 'class_start', 'class_end', 'class_name', 'class_teacherid','class_img','class_content','class_summary'],
        });
        if (detail){
          var detail=detail.dataValues;
          //评论20条
          var value=yield models.Classvalue.findAll({
            where:{value_classid:body.classid},
            limit:20,
            attributes:['value_content','value_votes'],
            include: [{
              model: models.Members,
              attributes:['m_name','m_pics','createdAt']
            }]
          });
          if(value){
          ...
          //课堂图片
          var retrospect=yield models.Retrospect.findAll({
            where:{ret_classid:body.classid},
            attributes:['ret_pics']
          });
          if (retrospect){
            retrospect.forEach(function(node,inde){
              node.dataValues.ret_pics=str.AbsolutePath(node.dataValues.ret_pics);
            })
          };
          //推荐书目
          var reference=yield models.Reference.findAll({
            where:{ref_classid:body.classid},
            attributes:['ref_pics','ref_content','ref_author']
          });
          if (reference){
          ...
          if (courseware){
            ...
            courseware.dataValues.cou_path=str.AbsolutePdfPath(courseware.dataValues.cou_path);
          };
          //讲师
          ...
          //状态  0未结束 4结束
          var report=0;
          if(moment(detail.class_end)<=moment()){
              class_ending=4
          }
          ...
          detail.retrospect=retrospect?retrospect:[];
    ...
    co(function*() {
      ...
      var item=yield models.Userclass.findAndCountAll({
        ...
        include: [{
          model: models.Goods,
          attributes: ['goodsid', 'goods_name', 'goods_img', 'goods_start','goods_teacher','goods_summary'],
          include: [{
            model: models.Class,
            where: content,
      ...
      if(item){
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (node.Good) {
            ...
            if (node.uc_status==1 && good.Classes) {
    ...
    var body=req.body;
    console.log(body)
    if (!body.goodsid || !body.headid || !body.calssroomid || !body.userphone) {
      return response.ApiError(res,{message:"goodsid or userphone or headid or calssroomid empty"});
    }
    //报名分为用户登陆报名和未登录报名
    ...
    co(function* (){
      try{
        ...
        var count=yield models.Userclass.findAndCountAll({
          where:{uc_calssroomid:body.calssroomid}
        })
        if(teuser){
        ...
        if (isuserclass){//未报名报名
          ...
          userclass=yield models.Userclass.create({
            ...
            uc_calssroomid:body.calssroomid,
            uc_userphone:body.userphone,
            uc_areaname:body.areaname,
            uc_calssroomname:body.calssroomname
          })
    ...
    var where={};
    co(function* (){
      var area = yield models.Area.findAll({
        where:where,
        attributes:['areaid','area_city'],
        include: [{
          model:models.Classroom,
          attributes:['classroom','classroom_area_city','classroom_head','classroom_address','classroom_name','classroom_longitude','classroom_coordinates']
        }]
      });
      return response.onListSuccess(res,{list:area});
    })
    ...
    if(body.classid)where.value_classid=body.classid;
    models.Classvalue.findAll({
      where:where,
      order:[['createdAt']],
      limit:options.pagesize,
      offset:options.offset,
      attributes:['value_content','value_votes','createdAt'],
      include: [{
