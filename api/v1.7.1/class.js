...
var Class={
  home:function(req,res){
    ...
    co(function * (){
      try{
        ...
        var middle=yield Promise.all(homeList.map(function(node,index){
          ...
          switch (type){
            ...
            case 2://活动
              ...
              sql.AppendFormat("select activity_stime as stime,activity_etime as etime,activity_id as id,activity_title as title,activity_img as img,activity_official as official,activity_address as address,activity_content as content,activity_link as `link` " +
            ...
            case 8://国际化
              ...
              sql.AppendFormat("select international_content as url,international_parent as id,international_title as title,international_img as img,international_summary as summary from gj_international where international_parent={0} ORDER BY international_id DESC LIMIT 0,4",arr[0])
              return models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT }).then(function(data){
                var map = {},
                    dest = [];
                for(var i = 0; i < data.length; i++){
            ...
            case 9://国际化
              ...
              sql.AppendFormat("select special_id,special_link as link,special_parent as id,special_title as title,special_img as img,special_summary as summary from gj_special where special_parent={0} and special_status=0 ORDER BY special_id DESC LIMIT 0,4",arr[0])
              return models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT }).then(function(data){
                var map = {},
                    dest = [];
                for(var i = 0; i < data.length; i++){
        ...
        // 缓存result;
        cache.set('home',result,config.redis.newTime,function (err,data) {
          if(err){
            console.log('设置缓存错误')
          }else {
            console.log('设置缓存成功')
  ...
  goods_detail:function(req,res){
    ...
    co(function *() {
      try{
        ...
        var sql=new StringBuilder();
        sql.AppendFormat("select m_name,goodsid,goods_name,goods_start,goods_summary,goods_img,goods_content,classid,class_img,class_name,class_start,class_end,class_qustart,class_teacher " +
            "from gj_goods " +
        ...
        models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT }).then(function(data) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            goods.name = node.goods_name;
            goods.img = node.goods_img;
            goods.summary = node.goods_summary;
            goods.time = str.getUnixToTime(node.goods_start);
  ...
  class_detail:function(req,res){
    ...
    co(function *() {
      try{
        var sql=new StringBuilder();
        sql.AppendFormat("select CASE WHEN prdid is NULL THEN 0 ELSE 1 END as video,topic_title,topic_id,mid,m_name,m_pics,m_title,class_value_link,m_desc,classid,class_qustatus,class_img,class_content,class_name,class_start,class_end,class_qustart,class_teacher,class_summary,goods_name " +
            "from gj_class " +
            "LEFT JOIN gj_topic ON gj_topic.topic_classid=gj_class.classid " +
            "LEFT JOIN gj_goods ON gj_goods.goodsid=gj_class.class_goodsid " +
            "LEFT JOIN gj_prdattach as prd on gj_topic.topic_id=prd.prdid AND prd.prd_type=40 " +
        ...
        var detail=yield models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT })
          ...
          for (var i = 0; i < detail.length; i++) {
            ...
            if(!map[node.classid]){
              ...
              var D={
                ...
                pics:node.class_img,
                title:node.class_name,
                goodsTitle:node.goods_name,
                time:node.class_start,
                endTime:node.class_end,
                content:node.class_content,
                summary:node.class_summary,
              ...
              if(node.topic_id){
                D.backList.push({id:node.topic_id,title:node.topic_title,isVideo:node.video})
              }
              dest.push(D);
              map[node.classid] = 'true';
            }else{
              for(var j = 0; j < dest.length; j++){
                ...
                if(dj.classid == node.classid){
                  ...
                  if(node.topic_id){
                    dj.backList.push({id:node.topic_id,title:node.topic_title,isVideo:node.video});
                  }
                  break;
                }
              }
          ...
          statusSql.AppendFormat("(select '书目' as title from gj_reference where ref_classid={0} limit 0,1) " +
              "union all " +
              "(select '课件' as title from gj_courseware where cou_classid={0} AND cou_transcoding=1 limit 0,1)",body.classid);
  ...
  topic_detail:function(req,res){
    ...
    co(function *() {
      try{
        ...
        var sql=new StringBuilder();
        sql.Append("select topic_classid as classid,prdattachid as pid,topic_id as id,topic_title as title,topic_content as content,prd_pics as img,attach_duration as duration,attach_path as path,prd_title from gj_topic");
        ...
        var detail= yield models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT })
          ...
          detail=dest[0];
          //话题要讲师名称，不该之前逻辑多增加取讲师的代码（后期优化需要合并）
          var teacherSql=new StringBuilder();
          teacherSql.AppendFormat("select m_name as name,class_summary as summary " +
              "from gj_class " +
              "INNER JOIN gj_members ON find_in_set(gj_members.mid,gj_class.class_teacher) where classid={0}",detail.classid);
          var teacher=yield models.sequelize.query(teacherSql.ToString(),{ type: models.sequelize.QueryTypes.SELECT });
          var teacherStr=''
          teacher.forEach(function (node) {
            teacherStr+=' / '+node.name
          })
          detail.teacher=teacherStr.substr(3);
          detail.summary=teacher[0].summary;
          return response.ApiSuccess(res, {detail: detail})
      }catch (err){
        console.log(err)
        return response.ApiError(res,{message:err.toString()})
  ...
  get_courseware:function(req,res){
    ...
    models.Courseware.findAll({
      where:{cou_classid:body.classid,cou_status:0,cou_transcoding:1},
  ...
  get_courseware_pdf:function (req,res) {
    var body=req.query;
    if(!body.path){
      return response.ApiError(res,{message:'参数缺失'})
    }
    if(body.path.indexOf('.pdf') == -1){
      return response.ApiError(res,{message:'路径解析错误'})
    }
    aly.getPdfToJpg(body.path,function (err,result) {
      if(!err && result.length>0){
        result.forEach(function (node,index) {
          result[index]=str.AbsolutePath(node.name)
        })
        function getNum(str) {
          var b=str.substring(str.lastIndexOf('/')+1)
          return b.substring(0,b.lastIndexOf('.'))
        }
        return response.ApiSuccess(res,{list:result.sort(function(a,b){
          return getNum(a)-getNum(b)
        })})
      }else {
        return response.ApiError(res,{message:'解析失败'})
      }
    })
  },
  ...
  get_international:function(req,res){
    var body=req.query;
    if(!body.id){
      return response.ApiError(res,{message:"参数缺失"})
    }
    var sql=new StringBuilder();
    sql.AppendFormat("select {0} as id,a.international_img as img,a.international_summary as content,a.international_title as title," +
        "b.international_img as itemImg,b.international_content as itemContent,b.international_title as itemTitle,b.international_summary as itemSummary " +
        "from gj_international as a " +
        "LEFT JOIN gj_international as b on a.international_id=b.international_parent " +
        "where a.international_id={0}",body.id);
    models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT }).then(function (data) {
      var map = {},
          dest = [];
      for(var i = 0; i < data.length; i++){
  ...
  get_splendid_video:function (req,res) {
    var body=req.query;
    if(!body.id){
      return response.ApiError(res,{message:'参数缺失'})
    }
    var sql=new StringBuilder();
    sql.Append("select prdattachid as id,prd_title as tag,prd_auther as title,prd_pics as img,at.attach_path as path from gj_prdattach as prd");
    sql.Append(" INNER JOIN gj_attach as at on at.attachid=prd.attachid");
    sql.AppendFormat(" where prd.prd_type=20");
    sql.AppendFormat(" ORDER BY prd.prd_desc*1 desc");
    models.sequelize.query(sql.ToString(),{ type: models.sequelize.QueryTypes.SELECT }).then(function(data){
      ...
      var detail=data.filter(function(node) {
        return node.id==body.id;
      })
      return response.ApiSuccess(res,{detail:detail[0]?detail[0]:data[0],list:data})
    }).catch(function (err) {
      console.log(err)
      return response.ApiError(res,{message:err.toString()})
    })
  },
  product_apply:function (req,res) {
    var body=req.body;
    var members;
    if(!body.uc_userphone || !body.goodsid || !body.uc_areaid || !body.uc_calssroomid || !body.m_name || !body.uc_status || !body.uc_calssroomname || !body.uc_areaname){
      return response.ApiError(res,{message:'参数缺失'})
    }
    co(function* (){
      try{
        ...
        if (!userclass){
          ...
          return response.ApiError(res, {message:'学员存在'});
        }
        //待修改
      }catch (err){
        console.log(err)
