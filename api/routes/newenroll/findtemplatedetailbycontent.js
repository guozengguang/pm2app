...
router.get('/', function (req, res) {
   var query = req.query,
    ...
    models.Apply_Template.findAndCountAll({
        ...
        sql.Append("select a.*,sum(case when b.vote is null then 0 else b.vote end) as vote from (select rs.createdAt ");
        ...
        result.rows.forEach(function(n,i){
            ...
            groupbystr+=' a.'+n.dataValues.name +',';
            listname.push(n.dataValues.name);
        });
        sql.AppendFormat(" from gj_apply_template_statistics as rs INNER JOIN gj_apply_template   as r on r.id = substring_index(substring_index(rs.parent,',',-2),',',1) LEFT JOIN  gj_apply_template as gr on gr.id = rs.`value` and r.method in (1,2) where rs.template = {0} and rs.status = 1",id);
     
        sql.Append(" group by rs.createdAt ORDER BY rs.createdAt  ) as a  ");
        
        sql.AppendFormat(" left join gj_template_vote as b on a.电话 = b.tophone and b.templateid = {0}  ",id);
        
        if(query.content)
        {
            sql.AppendFormat(" where a.姓名 like '%{0}%' " ,query.content);
        }
        sql.Append('  group by '+groupbystr+'a.createdAt  order by a.createdAt');
