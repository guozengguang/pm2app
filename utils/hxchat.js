...
var Hx= {
    ...
    requestToken: function () {
        return new Promise(function (resolev,reject) {
            cache.get("productionHxToken",function (err,data) {
                if(err){
                    reject(err)
                }else {
                    resolev(data)
            ...
            if(data){
                ...
                var headers = {
                    ...
                    client_id: config.hxchat.Client_Id,
                    client_secret: config.hxchat.Client_Secret
                };
                headers = JSON.stringify(headers);
                var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/token';
                var opt = {
                    method: "POST",
                    host: config.hxchat.host,
                    path: path,
                    headers: {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    var req = https.request(opt, function (res) {
                        ...
                        res.on('data', function (data) {
                            body += data;
                            if (res.statusCode == 200) {
                                cache.set('productionHxToken',data,3600*24*7);
                                return resolve(data);
                            } else {
                                return reject({code: res.statusCode, data: data});
                            }
...
module.exports = {
    ...
    reghxuser: function (option, callback) {
        ...
        data = JSON.stringify(data);
        var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/users';
    ...
    deletehxuser: function (option, callback) {
        Hx.requestToken().then(function (token) {
            ...
            data = JSON.stringify(data);
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/users/' + option.username;
    ...
    createhxgroup: function (option, callback) {
        Hx.requestToken().then(function (token) {
            ...
            data = JSON.stringify(data);
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/chatgroups';
    ...
    deletehxgroup: function (option, callback) {
        Hx.requestToken().then(function (token) {
            var data = '';
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/chatgroups/' + option.groupid;
    ...
    addhxgroupuser: function (option, callback) {
        Hx.requestToken().then(function (token) {
            var data = '';
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/chatgroups/' + option.groupid + '/users/' + option.username;
    ...
    sendmessages: function (option, callback) {
        Hx.requestToken().then(function (token) {
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/messages';
            var data = {
                target_type: 'users',
    ...
    createhxchatrooms: function (option, callback) {
        Hx.requestToken().then(function (token) {
            ...
            data = JSON.stringify(data);
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/chatrooms';
    ...
    setbacklist: function (option, callback) {
        Hx.requestToken().then(function (token) {
            ...
            data = JSON.stringify(data);
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/users/' + option.mid + '/blocks/users';
    ...
    deletebacklist: function (option, callback) {
        Hx.requestToken().then(function (token) {
            var data = ''
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/users/' + option.mid + '/blocks/users/' + option.username;
    ...
    addhxgroupuserbatch: function (option, callback) {
        Hx.requestToken().then(function (token) {
            ...
            data = JSON.stringify(data);
            var path = '/' + config.hxchat.org_name + '/' + config.hxchat.app_name + '/chatgroups/' + option.groupid + '/users/';
            var opt = {
                method: "POST",
                host: config.hxchat.host,
                path: path,
